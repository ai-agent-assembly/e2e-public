...
has its own real allow-path + deny-``xfail`` live module:
    ...
    langchain · langgraph · pydantic_ai · google_adk · haystack · llamaindex · mcp
    · openai_agents · smolagents · agno · microsoft_agent_framework

NOTE: crewai excluded (AAASM-4250) due to GHSA-f4j7-r4q5-qw2c in chromadb.
...
_SUPPORTED = [
    ...
    # crewai excluded: AAASM-4250 — chromadb has CRITICAL GHSA-f4j7-r4q5-qw2c with
    # no patched version. Re-add when chromadb ships a fix.
    ("google_adk", "agent_assembly.adapters.google_adk.adapter"),
    ("haystack", "agent_assembly.adapters.haystack.adapter"),
    ("llamaindex", "agent_assembly.adapters.llamaindex.adapter"),
...
#: Framework adapters excluded from coverage due to security vulnerabilities in
#: their transitive dependencies. These are NOT tested until the upstream patches.
_SECURITY_EXCLUDED = [
    # AAASM-4250: chromadb GHSA-f4j7-r4q5-qw2c (CRITICAL pre-auth code injection)
...
def test_no_unmapped_framework_adapters() -> None:
    ...
    and asserts each one is in ``_SUPPORTED`` or ``_SECURITY_EXCLUDED`` — so a
    newly-shipped framework adapter cannot land without a corresponding live smoke
    being demanded above. Security-excluded adapters are tracked but not tested
    until their transitive vulnerabilities are patched upstream.
    """
    ...
    mapped = {fw for fw, _ in _SUPPORTED} | {fw for fw, _ in _SECURITY_EXCLUDED}
