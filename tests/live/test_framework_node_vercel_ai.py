"""Live framework smoke test — **Vercel AI SDK** on the Node SDK (AAASM-3525).

Proves a *real* Vercel AI SDK agent runs end-to-end through the Node SDK's
governance path against a **live** ``aa-runtime`` — the production
``SDK → aa-ffi → aa-runtime`` transport, not a mock. The driver
(``drivers/node-frameworks/zod3-frameworks/vercel-ai.mjs``) builds a genuine
Vercel AI SDK ``tool`` and applies the SDK's ``patchVercelAiSdk`` hook (which
wraps the tool factory's ``execute`` with a pre-execution governance check;
Vercel tools have no ``.name`` so governance matches by description), then runs
...
``ToolCallIntercepted`` event is emitted over the UDS, and the Vercel AI SDK tool
...
def test_vercel_ai_enforcement_policy_is_well_formed() -> None:
...
def test_vercel_ai_allow_path_runs_governed_tool(live_runtime: LiveRuntime) -> None:
    """Allow path: a real Vercel AI SDK agent runs an allowed governed tool via live core.

    Drives the genuine Vercel AI SDK agent through the SDK's governance hook
...
# the restricted action is blocked for a Vercel AI SDK tool at the SDK layer.
@pytest.mark.xfail(
    strict=True,
    reason=(
        "AAASM-3172 (open flip-gate) tracks flipping this to a hard assert: "
        "live deny enforcement for a Vercel AI SDK agent is unprovable today because of "
...
def test_vercel_ai_deny_path_blocks_restricted_tool() -> None:
    """Deny path: a restricted Vercel AI SDK tool call is blocked against a live core.
    ...
    assert sdk_would_block is True, (
        f"Node SDK does not block {RESTRICTED_ACTION!r} for a Vercel AI SDK agent — "
