"""Live framework smoke test — **LangGraph.js** on the Node SDK (AAASM-3525).

Proves a *real* LangGraph.js agent runs end-to-end through the Node SDK's
governance path against a **live** ``aa-runtime`` — the production
``SDK → aa-ffi → aa-runtime`` transport, not a mock. The driver
(``drivers/node-frameworks/zod3-frameworks/langgraph.mjs``) builds a genuine
LangGraph ``StateGraph`` whose node calls a tool wrapped with the SDK's public
``withAssembly`` (and applies the LangGraph-specific ``patchLangGraph`` lineage
hook), then runs an **allowed** tool call.
...
``ToolCallIntercepted`` event is emitted over the UDS, and the LangGraph tool
...
def test_langgraph_enforcement_policy_is_well_formed() -> None:
...
def test_langgraph_allow_path_runs_governed_tool(live_runtime: LiveRuntime) -> None:
    """Allow path: a real LangGraph agent runs an allowed governed tool via live core.

    Drives the genuine LangGraph agent through the SDK's governance hook
...
# the restricted action is blocked for a LangGraph tool at the SDK layer.
@pytest.mark.xfail(
    strict=True,
    reason=(
        "AAASM-3172 (open flip-gate) tracks flipping this to a hard assert: "
        "live deny enforcement for a LangGraph.js agent is unprovable today because of "
...
def test_langgraph_deny_path_blocks_restricted_tool() -> None:
    """Deny path: a restricted LangGraph tool call is blocked against a live core.
    ...
    assert sdk_would_block is True, (
        f"Node SDK does not block {RESTRICTED_ACTION!r} for a LangGraph agent — "
